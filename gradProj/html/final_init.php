...
if($txt != ""){
    ...
    fwrite($myfile, $txt);
    fclose($myfile);
    shell_exec("./Assembler input.txt inter.txt");
    shell_exec("./Simulator_Pipelined inter.txt");
    $myfile = fopen("result.txt", "r");
    $i = 0;
    $numMemory = fgets($myfile, 1024);
    while($i < ($numMemory + 29)){
...
<html>
    ...
    <body>
        ...
        <div class="form-inline" style="height:450px">
           ...
           <div class="form-group text-center" style="position: absolute; left: 1050px; height:450px">
           ...
           <div class="form-group text-center" style="position: absolute; left: 1250px; height:450px">
           ...
           <div class="form-group text-center" style="position: absolute; left: 1500px; height:450px">
                <h2 class="text-center"> Binary Codes </h2>
                <textarea class="text-left" style="font-family: Arial narrow, Sans-serif; font-size:18px; width:200px; height:270px; resize:none">
                    <?php
                        if(($opt[0] == 3) || ($opt[1] == 3) || ($opt[2] == 3) || ($opt[3] == 3)){
                            $binCode = fopen("inter.txt", "r");
                            $j = 0;
                            while(!feof($binCode)){
                                $binary[$j] = fgets($binCode, 1024);
                                $j = $j + 1;
                            }
                            fclose($binCode);
                            $j = 0;
