	...
	ulint	rec_fold)  /*!< in: rec_fold of the page */
{
	...
	cell = hash_get_nth_cell(hash,
			hash_calc_hash(rec_fold, hash));
    node = (lock_t*)__sync_lock_test_and_set(&cell->node, in_lock); 
    in_lock->hash = node;
}
...
void
RecLock::lock_add2(lock_t* lock, bool add_to_hash)
{
	ut_ad(lock_mutex_own());
	ut_ad(trx_mutex_own(lock->trx));
	...
	if (add_to_hash) {
		...
		if (innodb_lock_schedule_algorithm == INNODB_LOCK_SCHEDULE_ALGORITHM_VATS
			...
			if (wait_lock) {
				...
				lock_rec_insert_to_head2(lock, m_rec_id.fold());
			}
		} else {
			HASH_INSERT2(lock_t, hash, lock_hash, key, lock);
		}
...
lock_t*
RecLock::create2(trx_t* trx, bool owns_trx_mutex, bool add_to_hash, const lock_prdt_t* prdt)
{
	return create2(NULL, trx, owns_trx_mutex, add_to_hash, prdt);
}
lock_t*
RecLock::create2(
	lock_t* const c_lock,
...
dberr_t
RecLock::add_to_waitq2(const lock_t* wait_for, const lock_prdt_t* prdt)
{
	ut_ad(lock_mutex_own());
	ut_ad(m_trx == thr_get_trx(m_thr));
	ut_ad(trx_mutex_own(m_trx));
	...
	/* Don't queue the lock to hash table, if high priority transaction. */
	lock_t*	lock = create2(m_trx, true, !high_priority, prdt);
					...
					transaction mutex */
{
	...
	if (lock == NULL && !(type_mode & LOCK_WAIT)) {
		...
		lock = lock_rec_find_similar_on_page2(
			type_mode, heap_no, first_lock, trx);
	...
	rec_lock.create2(trx, caller_owns_trx_mutex, true);
}
...
// 여기서 heap_no까지 아예 싸그리 검사
	...
	if (lock == NULL) {
		...
		if (!impl) {
			...
			rec_lock.create2(trx, false, true);
		}
...
// 마일스톤 1 에선 신경 안씀.
		if (wait_for != NULL) {
			...
			err = rec_lock.add_to_waitq2(wait_for);
...
/*    
    ...
    return(DB_SUCCESS_LOCKED_REC);
*/
 	lock_t*	lock = lock_rec_get_first_on_page2(heap_no, lock_sys->rec_hash, block);
	...
	if (lock == NULL) {
		...
		trx_mutex_enter(trx);
// 얘도 heap_no까지 싸그리 검사
		if (lock_rec_get_next_on_page2(lock)
		     || lock->trx != trx
		     || lock->type_mode != (mode | LOCK_REC)
		     || lock_rec_get_n_bits(lock) <= heap_no) {
