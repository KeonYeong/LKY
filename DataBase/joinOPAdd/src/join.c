    ...
    while(1){
        if(buffTable[clockHand].pin_count == 0 && buffTable[clockHand].table_id != 0){  // 버퍼내의 pin_count 가 0이면서, 비어있는 버프가 아닐경우
            if(!buffTable[clockHand].LRU){  // 해당 버퍼의 reference bit가 켜져있는지 본다.
                ...
                if(buffTable[clockHand].is_dirty) fileWrite(buffTable[clockHand].pageOff, &buffTable[clockHand].frame, tables[buffTable[clockHand].table_id - 1].fd);   // 우선 is_dirty인지 확인하여 디스크에 쓸 필요가 있는 지 확인한다. 필요할 경우 디스크에 쓴다.
...
// 실제 조인 함수, sort-merge를 사용한다 (b+ tree이기에)
int join_table(int table_id_1, int table_id_2, char* pathname){
    int i, j, mark;
    // 새로 쓰게 되는 파일의 descriptor
    int joiner;
    ...
    j = 0;
    while(1){
        // 만일 이미 i가 끝에 다다랐다면, 현 노드의 sibling node가 없을 경우 남은 결과버퍼를 파일에 마저 쓰고 조인을 종료시킨다. sibling이 있다면 옮긴다.
        ...
        //기준이 되는 테이블의 i를 올리면서 조인 테이블의 값과 같아질 떄까지 기다린다.
        while(buffTable[0].frame.rec.lRec[i].key < buffTable[1].frame.rec.lRec[j].key) {
            i++;
            //만약 현 노드를 초과할 경우 sibling으로 옮기고, sibling이 없다면 조인을 끝낸다.
            if(i == buffTable[0].frame.numKeys) {
                if(buffTable[0].frame.extraOff != 0){
                    fetchPage(buffTable[0].frame.extraOff, 0, table_id_1);
                    i = 0;
        ...
        // i 가 이제 같거나 더 클 경우이기에, 이젠 j를 올리게 된다.
        while(buffTable[0].frame.rec.lRec[i].key > buffTable[1].frame.rec.lRec[j].key) {
            j++;
            // 마찬가지로 만약 초과한다면 sibling으로 옮겨가고 sibling이 없다면 조인을 끝낸다.
        ...
        //이제 i와 j가 같다는 뜻이므로, j에 mark하고 i와 j의 키값이 같은동안 j를 하나씩 올리면서 계속 비교하고 같을 경우만  계속해서 join의 결과를 내뱉게 된다, 달라질 경우 i의 값을 올리며 j는 다시 mark로 돌리게 되고 같은 일을 수행한다. 아예 다른 값일 경우 다시 처음으로 돌아간다. 
        mark = j;
        while(buffTable[0].frame.rec.lRec[i].key == buffTable[1].frame.rec.lRec[j].key){
            while(buffTable[0].frame.rec.lRec[i].key == buffTable[1].frame.rec.lRec[j].key){
                joinOut(i, j, joiner);
...
// join의 결과를 쓰며 저장하는 함수, 일단 특별한 포맷으로 만들어진 string으로 저장해놓으며 버퍼에 붙히다가, 해당 버퍼가 꽉 차면 그제서야 그 버퍼를 파일에 쓰게 된다.
...
// 해당 버퍼를 파일에 쓰는 함수, 쓰고 나서는 해당 버퍼를 초기화 시켜주는 작업도 하게 된다.
...
int shutdown_db(){
    ...
    for(i = 0 ; i < buffNum; i ++){
        ...
        if(buffTable[i].table_id != 0){
            if(buffTable[i].is_dirty) fileWrite(buffTable[i].pageOff, &buffTable[i].frame, tables[buffTable[i].table_id - 1].fd);
...
int close_table(int table_id){
    ...
    for(i = 0 ; i < buffNum; i++){
        ...
        if(buffTable[i].table_id == table_id){
            if(buffTable[i].is_dirty) fileWrite(buffTable[i].pageOff, &buffTable[i].frame, tables[table_id - 1].fd);
            frame_init(i, 0, 0);
            currentInBuff--;
        }
    }
...
int open_table(char* pathname){
    ...
    if ((fd = open(pathname, O_RDWR | O_CREAT | O_EXCL | O_SYNC, 0644)) > 0){
        ...
        if(i == TABLENUM){
            ...
            fileRoot.numPages = 1;
            write(fd, &fileRoot, sizeof(h_page));
            return -1;
        }
        fileWrite(0, (page*)&tables[i].header, tables[i].fd);
	}
        ...
        for(i = 0 ; i < TABLENUM; i ++){
            if(!tables[i].inUse){
                if(!strcmp(tables[i].name, pathname)){ // case :: table is already Opened.
                    tables[i].fd = fd;
                    memcpy(&tables[i].header, &fileRoot, sizeof(h_page));
                    tableID = i + 1;
                    break;
                }
            }
        }
        if(i == TABLENUM){       // case :: tableID in header_page is not usable, allocate new
            for(i = 0; i < TABLENUM; i ++){
                if(!tables[i].inUse){
                    memset(&tables[i].header, 0, PGSIZE);
                    tables[i].fd = fd;
...
void insert_into_leaf_after_splitting(int table_id, int64_t key, char* value) {
	...
	if(tables[table_id - 1].header.freePageOff == 0){
		...
		ulint freeOff;
        page tmp;
        f_idx = bufferRead(tables[table_id - 1].header.freePageOff, &tmp, table_id);
        appointFrame(f_idx);
        freeOff = tmp.pageOff;
...
void adjust_root(int table_id) {
	...
	if (!buffPage->isLeaf) {
        ...
        int idx = bufferRead(tables[table_id - 1].header.rootPageOff, &tmp, table_id);
        appointFrame(idx);
        tmp.pageOff = 0;
