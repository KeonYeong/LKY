...
void headerWrite(){	// 헤드파일이 바뀌었으면 파일에 다시 씀
...
char* find(int64_t key){
	int i = 0;
	find_leaf(key); // 우선 key 가 들어있을 만한 leaf node를 찾는다.
...
int insert(int64_t key, char * value){
    ...
    if(currentOff != 0){
     	for (i = 0; i < buffPage->numKeys; i++){  // 이제 그 leaf node에서 해당 key에 맞는 데이터가 들어있는 지 찾는다!
	    	if (buffPage->rec.lRec[i].key == key) return -1;
        }
    }
	...
	insert_into_leaf_after_splitting(key, value);
	gv_init();
	headerWrite();
	return 0;
...
void insert_into_leaf_after_splitting(int64_t key, char* value) {
	...
	if(fileRoot.freePageOff == 0){
		...
		ulint freeOff;
        page tmp;
        fileRead(fileRoot.freePageOff, &tmp);
        freeOff = tmp.pageOff;
	...
	// 실제 저장하는 부분
	fileWrite(addedOff, neighbor);
	fileWrite(currentOff, buffPage);	
	// 새로운 키를 이제 부모에게 추가한다.
	insert_into_parent(new_key);
}
...
void insert_into_node_after_splitting(int left_index, int64_t key) {
    ...
    fileRead(neighbor->extraOff, &tmp);
    tmp.pageOff = addedOff;
	fileWrite(neighbor->extraOff, &tmp);
	for (i = 0; i < neighbor->numKeys; i++) {
		fileRead(neighbor->rec.iRec[i].pageOff, &tmp);
        tmp.pageOff = addedOff;
        fileWrite(neighbor->rec.iRec[i].pageOff, &tmp);		
	}
...
void insert_into_parent(int64_t key) {
	...
	if (currentOff == 0){
		insert_into_new_root(forLeftIndex, key);
		return;
	}
	...
	if (buffPage->numKeys < INTERNAL_ORDER -1){
		insert_into_node(left_index, key);
		return;
	}		

	// 하지만 만약 parent도 이미 꽉차 있다면..? parent 역시 스플릿 해야된다
	insert_into_node_after_splitting(left_index, key);
}
...
void insert_into_new_root(ulint leftOff, int64_t key) {
    ...
    fileWrite(currentOff, buffPage);
	fileRead(leftOff, &tmp);
    tmp.pageOff = currentOff;
    fileWrite(leftOff, &tmp);
...
void remove_entry_from_node(int64_t key) {
...
void adjust_root() {
	...
	if (!buffPage->isLeaf) {
        ...
        fileRead(fileRoot.rootPageOff, &tmp);
        tmp.pageOff = 0;
		fileWrite(fileRoot.rootPageOff, &tmp);
	}
...
void coalesce_nodes(int neighbor_index, int64_t k_prime) {
	...
	if (!buffPage->isLeaf) {
		...
		for (i = neighbor_insertion_index; i < neighbor->numKeys; i++) {
            fileRead(neighbor->rec.iRec[i].pageOff, &temp);
            temp.pageOff = remainingOff;
			fileWrite(neighbor->rec.iRec[i].pageOff, &temp);
		}
	}
	...
	// 이제 이웃 노드를 저장하고
	fileWrite(remainingOff, neighbor);
    memset(buffPage, 0, PGSIZE);
	...
	fileRead(currentOff, buffPage);
	// 부모 노드를 가리키게끔 바꿔준다.
	delete_entry(k_prime);// 그리고 부모에서 해당 키를 삭제
}
...
void redistribute_nodes(int neighbor_index, int k_prime_index, int64_t k_prime) {  
	...
	if (neighbor_index != -2) { // 만약 현 노드가 extraOff에 있는 게 아닌 경우, neighbor 바로 왼쪽
		if (!buffPage->isLeaf){	// 만약 현 노드가 인터널노드라면
            ...
            fileRead(buffPage->pageOff, &tmp);
			tmp.rec.iRec[k_prime_index].key = neighbor->rec.iRec[neighbor->numKeys - 1].key;
			fileWrite(buffPage->pageOff, &tmp);
			neighbor->rec.iRec[neighbor->numKeys - 1].key = 0;

            fileRead(buffPage->extraOff, &tmp);
            tmp.pageOff = currentOff;
			fileWrite(buffPage->extraOff, &tmp);
			// 그 후 저장
		}
			...
			fileRead(buffPage->pageOff, &tmp);
			tmp.rec.iRec[k_prime_index].key = buffPage->rec.lRec[0].key;
			fileWrite(buffPage->pageOff, &tmp);			
		}
	}
		...
		if (buffPage->isLeaf) {
			...
			fileRead(buffPage->pageOff, &tmp);
			tmp.rec.iRec[k_prime_index].key = neighbor->rec.lRec[0].key;
			fileWrite(buffPage->pageOff, &tmp);
            ...
            fileRead(buffPage->rec.iRec[buffPage->numKeys].pageOff, &tmp);
            tmp.pageOff = currentOff;
			fileWrite(buffPage->rec.iRec[buffPage->numKeys].pageOff, &tmp);

			fileRead(buffPage->pageOff, &tmp);
			tmp.rec.iRec[k_prime_index].key = neighbor->rec.iRec[0].key;
			fileWrite(buffPage->pageOff, &tmp);
...
void delete_entry(int64_t key) {
	...
	fileRead(buffPage->pageOff, &parent);
    ...
    fileRead(remainingOff, neighbor);
	...
	if (neighbor->numKeys + buffPage->numKeys < capacity){
		coalesce_nodes(neighbor_index, k_prime);
		return;
	}
	...
	// Redistribution.
		...
		redistribute_nodes(neighbor_index, k_prime_index, k_prime);
		return;
	}
}
