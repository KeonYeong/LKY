...
void print_file(){	// 파일 상태를 알고 싶어서 파일 데이터를 출력해주는 함수
	printf("######################################################\n");
	ulint size;
	size = lseek(ifd, 0, SEEK_END);
	printf("file size : %ld with %ld pages and %ld remains\n", size, size / PGSIZE, size % PGSIZE);

	lseek(ifd, 0, SEEK_SET);

	h_page* temp_head;
	temp_head = (h_page*)calloc(1, sizeof(h_page));
	read(ifd, temp_head, sizeof(h_page));

	printf("in Head \n");
	printf("\tfree_page_offset : %ld\n", temp_head->freePageOff);
	printf("\troot_page_offset : %ld\n", temp_head->rootPageOff);
	printf("\tnum_pages        : %ld\n", temp_head->numPages);
	free(temp_head);
	page *temp;
	temp = (page*)calloc(1, sizeof(page));

	while(1){
		read(ifd, temp, sizeof(page));
		printf("in page %ld\n", (lseek(ifd, 0, SEEK_CUR) - PGSIZE) / PGSIZE);
		...
		if(temp->isLeaf){
			int i;
			for(i = 0; i < 31; i++){
				if(temp->rec.lRec[i].key == 0) break;
				printf("\t[%d]key & vlaue             : [%ld][%s]\n",i, temp->rec.lRec[i].key, temp->rec.lRec[i].value);
			}
		}
		else{
			int i;
			for(i = 0; i < 248; i++){
				if(temp->rec.iRec[i].key == 0) break;
				printf("\t[%d]key & page_offset       : [%ld][%ld]\n",i, temp->rec.iRec[i].key, temp->rec.iRec[i].pageOff);
			}
		}
		if(lseek(ifd, 0, SEEK_CUR) == size) break;
	}
	free(temp);
	printf("######################################################\n\n\n");
}

void print_buffer(){    // 버퍼 상태를 알고 싶어서 버퍼 데이터를 출력해주는 함수
    printf("######################################################\n");
    printf("buff size : %d pages\n", buffNum);
    printf("Now we have : %d pages in Buff\n", currentInBuff);
    Buffer *temp;
    int j;
    int counter = 0;
    for(j=1; j <= buffNum; j++){
        temp = &buffTable[j - 1];
        if(temp->table_id == 0){
            if(currentInBuff == counter)break;
            else continue;
        }
        counter++;
        printf("in page %d\n", j);
        ...
        printf("\tnum_keys                   : %d\n", temp->frame.numKeys);
        printf("\tpage_offset                : %ld\n", temp->frame.extraOff);
        if(temp->frame.isLeaf){
            int i;
            for(i = 0; i < 31; i++){
                if(temp->frame.rec.lRec[i].key == 0) break;
                printf("\t[%d]key & vlaue : [%ld][%s]\n",i, temp->frame.rec.lRec[i].key, temp->frame.rec.lRec[i].value);
            }
        }
        else{
            int i;
            for(i = 0; i < 248; i++){
                if(temp->frame.rec.iRec[i].key == 0) break;
                printf("\t[%d]key & page_offset : [%ld][%ld]\n",i, temp->frame.rec.iRec[i].key, temp->frame.rec.iRec[i].pageOff);
            }
        }
        printf("\tpageOff is : %ld\n\ttable_id is : %d\n\tis_dirty is : %d\n\tpin_count is : %d\n\tLRU is : %d\n", temp->pageOff, temp->table_id, temp->is_dirty, temp->pin_count, temp->LRU);
    }
    printf("######################################################\n\n\n");
}
...
void headerWrite(int table_id){	// 헤드파일이 바뀌었으면 파일에 다시 씀
    tables[table_id - 1].header.freePageOff = fileRoot.freePageOff;
    tables[table_id - 1].header.rootPageOff = fileRoot.rootPageOff;
    tables[table_id - 1].header.numPages = fileRoot.numPages;
}
...
int bufferRead(ulint offset, page* buff, int table_id){
    ...
    if(i == buffNum){ // 만약 버퍼 풀에 해당 페이지가 존재하지 않는다면 적절한 policy로 하나의 페이지를 비우고 거기에 새로운 페이지를 디스크에서 읽어오게된다.
        ...
        fileRead(offset, &buffTable[idx].frame, tables[table_id - 1].ifd);  // 그 자리에 해당 페이지를 디스크에서 읽어온다.
        currentInBuff++;   
        memcpy(buff, &buffTable[idx].frame, PGSIZE);    // 그 페이지를 반환.
        return idx;
    }
    else return i;
}
...
int bufferWrite(ulint offset, page* buff, int table_id){
    if(bufferOn == 0){  //하는 일은 bufferRead에서 하는 일과 완벽히 똑같다. 
        fileWrite(offset, buff, tables[table_id - 1].ofd);
        return -1;
    }
    int i;
    for(i = 0; i < buffNum; i ++){
    ...
    if(i == buffNum){
        ...
        fileRead(offset, &buffTable[idx].frame, tables[table_id - 1].ifd);
        currentInBuff++;
    ...
    while(1){
        if(buffTable[clockHand].pin_count == 0 && buffTable[clockHand].table_id != 0){  // 버퍼내의 pin_count 가 0이면서, 비어있는 버프가 아닐경우
            if(!buffTable[clockHand].LRU){  // 해당 버퍼의 reference bit가 켜져있는지 본다.
                ...
                if(buffTable[clockHand].is_dirty) fileWrite(buffTable[clockHand].pageOff, &buffTable[clockHand].frame, tables[table_id - 1].ofd);   // 우선 is_dirty인지 확인하여 디스크에 쓸 필요가 있는 지 확인한다. 필요할 경우 디스크에 쓴다.
...
// 버퍼의 한 entity를 초기화 시키는 함수, 페이지를 0으로 비우고, 각 종 컨트롤 비트를 0 혹은 받아온 인자로 초기화 시킨다.
...
int shutdown_db(){
    ...
    for(i = 0 ; i < buffNum; i ++){
        ...
        if(buffTable[i].table_id != 0){
            if(buffTable[i].is_dirty) fileWrite(buffTable[i].pageOff, &buffTable[i].frame, tables[buffTable[i].table_id - 1].ofd);
    ...
    for(i = 0 ; i < TABLENUM; i ++){
        if(tables[i].inUse){
            fileWrite(0, (page*)&tables[i].header, tables[i].ofd);
        }
    }
    free(buffTable);
    free(tables);
...
int close_table(int table_id){
    ...
    for(i = 0 ; i < buffNum; i++){
        ...
        if(buffTable[i].table_id == table_id){
            if(buffTable[i].is_dirty) fileWrite(buffTable[i].pageOff, &buffTable[i].frame, tables[table_id - 1].ofd);
            frame_init(i, 0, 0);
            currentInBuff--;
        }
    }
    fileWrite(0, (page*)&tables[table_id - 1].header, tables[table_id - 1].ofd);
    tables[table_id - 1].inUse = 0;
    memset(tables[table_id - 1].name, 0, 120);
    tables[table_id - 1].ofd = 0;
    tables[table_id - 1].ifd = 0;
    return 0;
} 
...
int open_table(char* pathname){
    ...
    if ((ofd = open(pathname, O_WRONLY | O_CREAT | O_EXCL | O_SYNC, 0644)) > 0){
        if((ifd = open(pathname, O_RDONLY | O_SYNC)) == -1) return -1;
		fileRoot.numPages = 1;
        for(i = 0 ; i < TABLENUM; i ++){
            if(!tables[i].inUse){
                fileRoot.tableID = i + 1;
                tables[fileRoot.tableID - 1].ofd = ofd;
                tables[fileRoot.tableID - 1].ifd = ifd;
                tables[fileRoot.tableID - 1].inUse = 1;
                strcpy(tables[fileRoot.tableID - 1].name, pathname);
                break;
            }
        }
        if(i == TABLENUM){
            printf("You have opened more than 10 tables, close one table first\n");
            lseek(ofd, 0, SEEK_SET);
            write(ofd, &fileRoot, sizeof(h_page));
            return -1;
        }
        memcpy(&tables[fileRoot.tableID - 1].header, &fileRoot, sizeof(h_page));
        fileWrite(0, (page*)&fileRoot, tables[fileRoot.tableID - 1].ofd);
	}
	else {
		// 파일이 존재한다면 염
		if((ofd = open(pathname, O_WRONLY | O_SYNC)) == -1) return -1;
    	if((ifd = open(pathname, O_RDONLY | O_SYNC)) == -1) return -1;
        lseek(ifd, 0, SEEK_SET);
		read(ifd, &fileRoot, sizeof(h_page));
        if(fileRoot.tableID != 0 && (!tables[fileRoot.tableID - 1].inUse || !strcmp(tables[fileRoot.tableID - 1].name, pathname))){ // case :: tableID in header_page is usable;
            tables[fileRoot.tableID - 1].ofd = ofd;
            tables[fileRoot.tableID - 1].ifd = ifd;
            tables[fileRoot.tableID - 1].inUse = 1;
            strcpy(tables[fileRoot.tableID - 1].name, pathname);
        }
        else{       // case :: tableID in header_page is not usable, allocate new
            for(i = 0; i < TABLENUM; i ++){
                if(!tables[i].inUse){
                    fileRoot.tableID = i + 1;
                    tables[fileRoot.tableID - 1].ofd = ofd;
...
void find_leaf(int table_id, int64_t key) {
    ...
    int idx = 0;
	ulint temp = 0;
	currentOff = fileRoot.rootPageOff; // 루트부터 검색 시작
	if (currentOff == 0) { // 루트가 0이면 빈 나무인것
		return;
	}
    idx = bufferRead(currentOff, buffPage, table_id);
    appointFrame(idx);
...
void insert_into_leaf_after_splitting(int table_id, int64_t key, char* value) {
    ...
    neighbor->isLeaf = 1;
	if(fileRoot.freePageOff == 0){
		addedOff = fileRoot.numPages * PGSIZE;
		fileRoot.numPages++;
	}
	else {
		ulint freeOff;
        page tmp;
        f_idx = bufferRead(fileRoot.freePageOff, &tmp, table_id);
        appointFrame(f_idx);
        freeOff = tmp.pageOff;
		addedOff = fileRoot.freePageOff;
		fileRoot.freePageOff = freeOff;
	}
	insertion_index = 0;
	// 해당 위치를 찾는다
...
void insert_into_node_after_splitting(int table_id, int left_index, int64_t key) {
    ...
    int idx = bufferRead(neighbor->extraOff, &tmp, table_id);
    appointFrame(idx);
    tmp.pageOff = addedOff;
	bufferWrite(neighbor->extraOff, &tmp, table_id);
    terminateFrame(idx);
	for (i = 0; i < neighbor->numKeys; i++) {
		idx = bufferRead(neighbor->rec.iRec[i].pageOff, &tmp, table_id);
        appointFrame(idx);
        tmp.pageOff = addedOff;
...
void insert_into_parent(int table_id, int64_t key) {
	...
	usingIndex = bufferRead(currentOff, buffPage, table_id);
...
void insert_into_new_root(int table_id, ulint leftOff, int64_t key) {
	...
	if(fileRoot.freePageOff == 0){
		fileRoot.rootPageOff = fileRoot.numPages * PGSIZE;
		currentOff = fileRoot.rootPageOff;
		fileRoot.numPages++;
	}
	else{
		ulint freeOff;
		fileRoot.rootPageOff = fileRoot.freePageOff;
		currentOff = fileRoot.rootPageOff;
        f_idx = bufferRead(fileRoot.freePageOff, &tmp, table_id);
        appointFrame(f_idx);
        freeOff = tmp.pageOff;
    ...
    bufferWrite(currentOff, buffPage, table_id);
    if(f_idx != 0)terminateFrame(f_idx);
	int idx = bufferRead(leftOff, &tmp, table_id);
    appointFrame(idx);
...
void adjust_root(int table_id) {
	...
	if (!buffPage->isLeaf) {
        page tmp;
		fileRoot.rootPageOff = buffPage->extraOff;
        int idx = bufferRead(fileRoot.rootPageOff, &tmp, table_id);
        appointFrame(idx);
        tmp.pageOff = 0;
		bufferWrite(fileRoot.rootPageOff, &tmp, table_id);
        terminateFrame(idx);
	}

	// 만약 루트가 리프 노드라면 -> 자식도 없는데 본인은 key도 없다, 그러면 나무 전체가 빈거다, 따라서 그냥 나무 자체를 없다고 한다.
	else {
		fileRoot.rootPageOff = 0;
	}
	// 나무의 옛 루트를 초기화 시키고 프리리스트에 추가한다.
	memset(buffPage, 0, PGSIZE);
	if(fileRoot.freePageOff != 0) buffPage->pageOff = fileRoot.freePageOff;
	fileRoot.freePageOff = currentOff;
	bufferWrite(currentOff, buffPage, table_id);
}
...
void coalesce_nodes(int table_id, int neighbor_index, int64_t k_prime, int n_idx, page* parent, int p_idx) {
	...
	if (!buffPage->isLeaf) {
		...
		for (i = neighbor_insertion_index; i < neighbor->numKeys; i++) {
            idx = bufferRead(neighbor->rec.iRec[i].pageOff, &temp, table_id);
            appointFrame(idx);
	...
	if(fileRoot.freePageOff != 0)freeOff = fileRoot.freePageOff;
	fileRoot.freePageOff = currentOff;
...
void redistribute_nodes(int table_id, int neighbor_index, int k_prime_index, int64_t k_prime, page * parent) {  
	...
	if (neighbor_index != -2) { // 만약 현 노드가 extraOff에 있는 게 아닌 경우, neighbor 바로 왼쪽
		if (!buffPage->isLeaf){	// 만약 현 노드가 인터널노드라면
            ...
            idx = bufferRead(buffPage->extraOff, &tmp, table_id);
            appointFrame(idx);
            tmp.pageOff = currentOff;
		...
		if (buffPage->isLeaf) {
            ...
            idx = bufferRead(buffPage->rec.iRec[buffPage->numKeys].pageOff, &tmp, table_id);
            appointFrame(idx);
            tmp.pageOff = currentOff;
...
void delete_entry(int table_id, int64_t key) {
	...
	if (currentOff == fileRoot.rootPageOff) {
	...
	int p_idx = bufferRead(buffPage->pageOff, &parent, table_id);
    appointFrame(p_idx);
    ...
    int n_idx = bufferRead(remainingOff, neighbor, table_id);
    appointFrame(n_idx);
